...
import { useStakeContractE, useStakeContract } from "../../hooks/useContract";
...
const Home = () => {
  ...
  const handleStake = async () => {
    ...
    try {
      setLoading(true);
      /** ethers  */
       const tx = await stakeContractE.depositETH({value: parseUnits(amount, 18) })
      await tx.wait();
      console.log(`交易详情：`,tx);
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      const tx = await stakeContractE.claim([Pid]);
      // const res = await waitForTransactionReceipt(data, { hash: tx });
      tx.wait();
      if (tx.data){
