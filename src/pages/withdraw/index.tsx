...
import { useStakeContract, useStakeContractE } from "../../hooks/useContract";
...
const Withdraw = () => {
  ...
  const stakeContractE = useStakeContractE();
  ...
  const getUserData = useCallback(async () => {
    /**ethers */
     if (!stakeContractE || !address) return;
    const staked = await stakeContractE.stakingBalance([Pid, address]);
    // const staked = await stakeContract.read.stakingBalance([Pid, address]);
    console.log("staked=====>", staked)
    // @ts-ignore
    const [requestAmount, pendingWithdrawAmount] = await stakeContractE.withdrawAmount([Pid, address]);
  ...
  useEffect(() => {
    ...
    if (stakeContractE && address) {
      getUserData();
    }
  }, [address, stakeContractE, getUserData]);
  // }, [address, stakeContract, getUserData]);

  const handleUnStake = useCallback(async () => {
    if (!stakeContractE || !data) return;
    ...
    try {
      ...
      /** ethers */
      const tx = await stakeContractE.unstake([Pid, parseUnits(amount, 18)]);
      tx.wait()
      /** end ethers */
      /** viem */
   ...
   }, [stakeContractE, data, amount, userData.staked, getUserData]);
  ...
  const handleWithdraw = useCallback(async () => {
    ...
    try {
      ...
      const tx = await stakeContractE.withdraw([Pid]);
      await waitForTransactionReceipt(data, { hash: tx });
  ...
  }, [stakeContractE, data, getUserData]);
