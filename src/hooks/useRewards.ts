...
import { useStakeContract, useStakeContractE, useStakeContractER } from './useContract';
  ...
  const fetchPoolData = useCallback(async () => {
    ...
    try {
      const pool = await retryWithDelay(() => 
        ...
        // stakeContract.pool([Pid]) as Promise<PoolData>
      );
  ...
  const fetchMetaNodeAddress = useCallback(async () => {
    ...
    try {
      const address = await retryWithDelay(() => 
        ...
        // stakeContract.MetaNode() as Promise<string>
      );
  ...
  const fetchRewardsData = useCallback(async () => {
    ...
    try {
      ...
      const userData = await retryWithDelay(() => 
        ...
        // stakeContract.user([Pid, address]) as Promise<UserData>
      );
      const stakedAmount = await retryWithDelay(() => 
        stakeContract.read.stakingBalance([Pid, address]) as Promise<bigint>
        // stakeContract.stakingBalance([Pid, address]) as Promise<bigint>
      );
